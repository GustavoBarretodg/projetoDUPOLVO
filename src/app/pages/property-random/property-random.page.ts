...
export class PropertyRandomPage implements OnInit {
  ...
  async onSave(qtdCard: any) {
    ...
    const alert = await this.alertCtrl.create({
      ...
      message: `<strong>${qty} cartão(ns)</strong> de ${this.gameConfig.name} totalizando <strong>${formatBRL(total)}</strong>.<br><br>Deseja adicionar ao carrinho? O pagamento será confirmado pelo administrador.`,
      buttons: [
        { text: 'Cancelar', role: 'cancel' },
        { text: 'Adicionar', handler: () => this.saveRandomBet(qtdCard) }
      ]
    });
    await alert.present();
  }

  private saveRandomBet(qtdCard: any) {
    const params: any = {
      id_bet: 1030,
